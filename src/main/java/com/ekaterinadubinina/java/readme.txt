...
Для получения данных по пользователям посылаются следующие запросы:
При HTTP GET запросе http://localhost:8080/api/persons/ выводятся все пользователи с их данными:
{
        "id": 1,
        "firstName": "Ekaterina",
        "lastName": "Dubinina",
...
При HTTP GET запросе http://localhost:8080/api/persons/id выводится пользователь с указанным id. Так, если необходимо узнать информацию по пользователю с id= 4, посылается запрос http://localhost:8080/api/persons/4 :
{
        "id": 4,
        "firstName": "Svetlana",
        "lastName": "Minina",
...
При HTTP GET запросе http://localhost:8080/api/persons/id/notes выводятся все объявления пользователя:
 {
        "id": 4,
        "personId": 4,
        "headline": "Nail art",
...
При HTTP POST запросе http://localhost:8080/api/persons/ создается пользователь, необходимо указать данные пользователя:
{
    "firstName": "Kira",
    "lastName": "Volkokova",
...
При HTTP DELETE запросе http://localhost:8080/api/persons/ удаляется пользователь с указанным id:
Получаемый ответ:
"Person deleted"

Если необходимо получить информацию по объявлениям посылаются следующие запросы:
При HTTP GET запросе http://localhost:8080/api/notes/ все созданные объявления:
 {
        "id": 1,
        "personId": 1,
...
…
При HTTP GET запросе http://localhost:8080/api/notes/id выводится объявление по указанному идентификатору объявления:
{
    "id": 4,
    "personId": 4,
    "headline": "Nail art",
...
При HTTP POST запросе http://localhost:8080/api/notes/ создается новое объявление, необходимо указать все данные для публикации:
{
   "personId": "2",
...
При HTTP DELETE запросе http://localhost:8080/api/notes/id/ удаляется объявление  с указанным id:
Получаемый ответ:
"Note deleted"
...
• Postgresql: База данных была реализована посредством https://www.postgresql.org/ .
URL: jdbc:postgresql://localhost:5432/note

USER=postgres
PASSWORD=1234

• Maven: Управление зависимостями, архитектура и сборка проекта.
• Postman: Отображение данных при тестировании запросов.
